...
@property(nonatomic,strong)NSMutableArray *pictureNameArray; //存放图片名称
@property(nonatomic,assign)int arrayIndex; //存放数组下标，默认是0
@property(nonatomic,strong)NSTimer *timer; //定时器，根据定时器时间触发事件，达到图片无限滚动的效果
...
- (void)viewDidLoad {
    ...
    //往数组里面添加图片
    self.pictureNameArray = [NSMutableArray arrayWithArray:@[@"1.jpg",@"2.jpg",@"3.jpg",@"4.jpg",@"5.jpg"]];
...
- (UIPageControl *)pageControl
{
    if (!_pageControl)
    {
        CGFloat floatX = self.view.bounds.size.width/2 - _pageControl.bounds.size.width/2 ; // 居中显示
        CGFloat floatY = 64 + 200 - 20 ; // 距离scrollView最底部20处
        _pageControl = [[UIPageControl alloc] initWithFrame:CGRectMake(floatX, floatY, 0, 0)] ;
        // 设置小圆点的个数
